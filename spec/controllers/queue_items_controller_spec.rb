...
describe QueueItemsController do
  ...
  describe "POST create" do
    ...
    it "creates the queue item that is asscoiciated with the video" do
      session[:user_id] = Fabricate(:user)
      video = Fabricate(:video)
      post :create, video_id: video.id
    ...
    it "creates the queue item that is asscoiciated with the signed in user" do
      alice = Fabricate(:user)
      session[:user_id] = alice.id
      video = Fabricate(:video)
      post :create, video_id: video.id
    ...
    it "redirects to the log in for unauthentcated users" do
      monk = Fabricate(:video)
  ...
  describe "POST update_queue" do
    ...
    context "with invalid input" do
      it "redirects back to my_queue_path" do
        alice = Fabricate(:user)
        session[:user_id] = alice.id
        queue_item1 = Fabricate(:queue_item, user: alice, position: 1)
        queue_item2 = Fabricate(:queue_item, user: alice, position: 2)
